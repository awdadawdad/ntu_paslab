...
class Phi3LongRoPEScaledRotaryEmbedding(nn.Module):
    ...
    def __init__(self, dim, args: ModelArgs, device):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, dtype):
        ...
        return (
            self.cos_cached[: self.max_position_embeddings].to(dtype=dtype),
            self.sin_cached[: self.max_position_embeddings].to(dtype=dtype),
        )
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs, li: int):
        ...
        self.o_proj = nn.Linear(args.n_heads * args.head_dim, args.dim, bias=True)
        self.rotary_emb = Phi3LongRoPEScaledRotaryEmbedding(self.head_dim, args, device=args.device)
...
class Phi3MoE:
    ...
    def build(model_path: str, node_id: int, device: torch.device) -> "Phi3MoE":
        ...
        if (
            non_experts[f"model.layers.0.self_attn.q_proj.weight"].shape[0]
...
def main(
    ...
    model = Phi3MoE.build(model_path, node_id, gpu)
