...
class PhiMoEDecoderLayer(nn.Module):
    def __init__(self, config: PhiMoEConfig, layer_idx: int, group):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class PhiMoEModel(PhiMoEPreTrainedModel):
    ...
    def __init__(self, config: PhiMoEConfig, group):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        self.layers_per_gpu = config.num_hidden_layers//WORLD_SIZE
        if WORLD_RANK == 0:
            self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList(
            [PhiMoEDecoderLayer(config, layer_idx, group) for layer_idx in range((self.layers_per_gpu)* WORLD_RANK,  (WORLD_RANK +1)*(self.layers_per_gpu))]
        )
...
class PhiMoEForCausalLM(PhiMoEPreTrainedModel):
    ...
    def __init__(self, config, group):
        super().__init__(config)
        self.model = PhiMoEModel(config, group)
        self.vocab_size = config.vocab_size
        if WORLD_RANK == WORLD_SIZE -1:
    ...
    def load(model_path: Path, gpu: torch.device, group) -> "PhiMoEModel":
        ...
        if WORLD_RANK == 0:
            embed_tokens = torch.load(
                model_path / "embed_tokens.pt",
                map_location=gpu,
                weights_only=True,
                mmap=True,
            )
        if WORLD_RANK == WORLD_SIZE -1:
            lm_head = torch.load(
                model_path / "lm_head.pt",
                map_location=gpu,
                weights_only=True,
                mmap=True,
            )
        weight = torch.load(
            model_path / f"gpu{WORLD_RANK}.pt",
            map_location=gpu,
            weights_only=True,
            mmap=True,
        )
